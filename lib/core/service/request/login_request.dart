...
class HYLoginRequest {
  ...
  static Future<HYHashKeyModel> getPassportLogin() async {
    const _url = "/x/passport-login/web/key?build=6720300&platform=android";
    final result = await HttpBaseRequest.request("login", _url);
    return HYHashKeyModel.fromJson(result);
  }

  ///短信登录
  static Future<String> messageCodeLogin(_postBody) async {
    const _url = "/x/passport-login/login/sms";
    final result = await HttpBaseRequest.request("login", _url,
        params: _postBody,
        method: 'POST',
        contentType: 'UTF-8',
  ...
  ///发送短信
  static Future<String> sendSMSMessage(_postBody) async {
    const _url = "/x/passport-login/sms/send";
    final result = await HttpBaseRequest.request("login", _url,
        params: _postBody,
        method: 'POST',
        contentType: 'UTF-8',
