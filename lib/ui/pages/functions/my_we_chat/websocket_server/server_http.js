...
app.post('/wechat/register', (req, res) => {
    ...
    if (util.strIsNotEmpty(tel) && util.strIsNotEmpty(userName) && util.strIsNotEmpty(password)) {
        ///查询是否存在用户
        connection.query(
            `SELECT * FROM mybilibili.my_wechat_user where tel = \"${tel}\"`,
            function (err, rows, fields) {
                ...
                if (rows.length != 0) {
                    ...
                    ///新增用户
                    connection.query(
                        `INSERT INTO mybilibili.my_wechat_user (\`userId\`, \`userName\`, \`tel\`, \`password\`) VALUES (\"${userId}\", \"${userName}\", \"${tel}\", \"${password}\")`,
                        function (err, rows, fields) {
                            if (err) throw err
                            res.send({
                                "code": 0,
...
app.post('/wechat/login', (req, res) => {
    ...
    if (util.strIsNotEmpty(tel) && util.strIsNotEmpty(password)) {
        ///查询是否存在用户
        connection.query(
            `SELECT * FROM mybilibili.my_wechat_user where tel = \"${tel}\" && password = \"${password}\"`,
            function (err, rows, fields) {
                if (err) throw err
                if (rows.length != 0) {
                    res.send({
...
app.get('/wechat/users', (req, res) => {
    connection.query(`SELECT userId, userName, avatar FROM mybilibili.my_wechat_user`, function (err, rows, fields) {
        if (err) throw err
        res.send({
            "code": 0,
            "message": "获取用户列表成功",
